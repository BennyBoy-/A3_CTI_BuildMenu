...
CTI_Coin_LoadSubMenu = {
	...
	if ("CTI_COIN_Items_0" in _categories) then {
		...
		["Base", "CTI_COIN_Items_0", [_itemVariable, _items, _itemEnabled], "", "missionNamespace setVariable ['CTI_COIN_PARAM', %1]; missionNamespace setVariable ['CTI_COIN_PARAM_KIND', 'STRUCTURES']"] call BIS_FNC_createmenu;
	};
	...
	if ("CTI_COIN_Items_1" in _categories) then {
		...
		
		{
			...
			_sub_itemVariable = [];
			{
				...
				_sub_itemEnabled pushBack true;
				_sub_itemVariable pushBack _x;
			} forEach (missionNamespace getVariable format["CTI_COIN_DEFENSE_CATEGORY_%1", _x]);
			
			[_category, format["CTI_COIN_SubItem_%1", _x], [_sub_itemVariable, _sub_items, _sub_itemEnabled], "", "missionNamespace setVariable ['CTI_COIN_PARAM', %1]; missionNamespace setVariable ['CTI_COIN_PARAM_KIND', 'DEFENSES']"] call BIS_FNC_createmenu;
...
CTI_Coin_OnMouseZChanged = {
	with missionNamespace do {
		if !(isNil "CTI_COIN_PREVIEW") then {
			...
			CTI_COIN_PREVIEW setDir (direction CTI_COIN_PREVIEW + _angle);
		};
	};
};
